...
def get_v36_analysis(symbol):
    ...
    data = None
    # محاولة جلب البيانات من Binance و MEXC
    ...
    if not data or len(data) < 50:
        return f"⚠️ العملة `{s}` غير موجودة أو المنصة محجوبة حالياً.", None
    ...
    try:
        ...
        if p > ema and rsi > 52 and vols[-1] > vol_avg:
            sig, pred, emo = "🚀 صعود قوي (LONG)", "خضراء صاعدة 📈", "🟢"
            t1, t2, sl = p*1.03, p*1.07, p*0.96
        elif p < ema and rsi < 48:
            sig, pred, emo = "📉 هبوط متوقع (SHORT)", "حمراء هابطة 📉", "🔴"
    ...
    except: return "⚠️ خطأ في معالجة بيانات العملة.", None

# --- [ قسم الشحن التلقائي واليدوي - Callback ] ---
@bot.callback_query_handler(func=lambda call: True)
def handle_callbacks(call):
    uid = call.message.chat.id
    if call.data == "pay_auto":
        msg = bot.send_message(uid, "💰 أدخل المبلغ المراد شحنه ($):")
        bot.register_next_step_handler(msg, create_invoice_logic)
        ...
        save_db()
        bot.send_message(int(tid), f"✅ تم تفعيل اشتراك VIP بـ {amt}$ بنجاح!")
        bot.edit_message_caption("✅ تم التفعيل بنجاح للمستخدم.", OWNER_ID, call.message.message_id)

def create_invoice_logic(m):
    try:
        amt = float(m.text)
        res = requests.post("https://api.oxapay.com/merchants/request", json={'merchant': OXAPAY_KEY, 'amount': amt, 'currency': 'USD', 'description': f"VIP_{m.chat.id}"}).json()
        if res.get('payLink'):
            mk = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("💳 رابط الدفع الآمن", url=res['payLink']))
            bot.send_message(m.chat.id, f"📝 فاتورة شحن بـ {amt}$:\nسيتم التفعيل آلياً بعد الدفع.", reply_markup=mk)
            # خيط لمراقبة الدفع (اختياري) يمكن إضافته هنا
    except: bot.send_message(m.chat.id, "⚠️ يرجى إدخال رقم صحيح (مثال: 50).")

def wait_for_receipt(m):
    if m.photo:
        mk = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton(f"✅ تفعيل الاشتراك", callback_data=f"adm_confirm_50_{m.chat.id}"))
        bot.send_photo(OWNER_ID, m.photo[-1].file_id, caption=f"🔔 طلب شحن يدوي من: `{m.chat.id}`", reply_markup=mk)
        bot.send_message(m.chat.id, "✅ تم إرسال الإيصال للمراجعة. سيتم تفعيلك قريباً.")
    else: bot.send_message(m.chat.id, "❌ لم يتم إرسال صورة. حاول مجدداً.")
...
def start(m):
    ...
    bot.send_message(m.chat.id, "📊 **مرحباً بك في نظام رادار V36 المطور**\nاختر من القائمة أدناه للبدء:", reply_markup=main_menu())
...
def ana_init(m):
    ...
    if not is_vip and db["free_usage"].get(uid, 0) >= 3:
        return bot.send_message(m.chat.id, "❌ انتهت المحاولات المجانية. يرجى الشحن لتفعيل VIP.")
...
def ana_execute(m):
    ...
    bot.send_message(m.chat.id, "🔍 **جاري تشريح الشمعات والسيولة...**")
