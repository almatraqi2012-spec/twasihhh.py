...
@app.route('/')
def home(): 
    return "RADAR SYSTEM V42 IS FULLY ACTIVE AND ONLINE"
...
# --- [ 3. محرك التحليل الاحترافي V42 - دقة واقعية ] ---
def get_v42_analysis(symbol):
    ...
    try:
        ...
        # حساب المتوسطات والانحراف
        ema = sum(closes[-20:]) / 20
        std_dev = (sum([(x - ema)**2 for x in closes[-20:]]) / 20)**0.5
        macd = (sum(closes[-12:]) / 12) - (sum(closes[-26:]) / 26)
        
        # Bollinger Bands للأهداف
        upper_band = ema + (std_dev * 2)
        lower_band = ema - (std_dev * 2)

        # منطق التحليل - فلاتر الأمان ضد التوصيات الكاذبة
        if p > ema and macd > 0 and 50 < rsi < 68:
            sig, emo, txt = "🚀 دخول شراء (LONG)", "🟢", "السيولة إيجابية والزخم صحي للدخول."
            t1, t2, sl = p + (std_dev * 1.5), upper_band, p - (std_dev * 1.8)
            
        elif p < ema and macd < 0 and 32 < rsi < 48:
            sig, emo, txt = "📉 دخول بيع (SHORT)", "🔴", "ضغط بيعي واضح مع كسر مستويات المتوسط."
            t1, t2, sl = p - (std_dev * 1.5), lower_band, p + (std_dev * 1.8)
            
        elif rsi >= 70:
            sig, emo, txt = "⚠️ تشبع شرائي (قمة)", "🟡", "العملة متضخمة جداً! خطر الشراء من القمة حالياً."
            t1, t2, sl = p * 1.01, upper_band * 1.01, p * 0.98
            
        elif rsi <= 30:
            sig, emo, txt = "⚠️ تشبع بيعي (قاع)", "🔵", "نزيف بيعي حاد، توقع ارتداد تصحيحي قريباً."
            t1, t2, sl = p * 1.03, p * 1.06, p * 0.96
            
        else:
            sig, emo, txt = "⏳ منطقة حيرة", "⚪", "تذبذب عرضي، لا توجد سيولة كافية لتأكيد الاتجاه."
            t1, t2, sl = p * 1.015, p * 1.03, p * 0.98

        chart_link = f"https://www.tradingview.com/chart/?symbol={source_name}:{s}"
        
        return (f"🏛 **رادار القابضة - التقرير الفني V42**\n━━━━━━━━━━━━━━\n"
...
# --- [ 4. نظام الشحن ومعالجة الأزرار والطلبات ] ---
...
def handle_query(call):
    ...
    if call.data == "pay_auto":
        ...
        bot.send_message(uid, f"📌 حول 50$ لعنوان المحفظة (USDT.TRC20):\n`{MY_WALLET}`\n\nثم أرسل صورة الإيصال هنا.")
...
def create_invoice(m, amt):
    try:
        payload = {'merchant': OXAPAY_KEY, 'amount': amt, 'currency': 'USD', 'description': str(m.chat.id)}
...
def wait_for_receipt(m):
    if m.photo:
        ...
        bot.send_photo(OWNER_ID, m.photo[-1].file_id, 
                       ...
                       reply_markup=mk)
        bot.send_message(m.chat.id, "✅ تم استلام الإيصال، سيتم مراجعته قريباً.")
    else:
...
def dep_menu(m):
    ...
    bot.send_message(m.chat.id, "اختر وسيلة الشحن المناسبة (50$):", reply_markup=mk)
...
def ana_init(m):
    ...
    msg = bot.send_message(m.chat.id, "🎯 أرسل رمز العملة (مثال: BTC, SOL):")
...
def ana_execute(m):
    ...
    if res:
        ...
        bot.send_message(m.chat.id, "⚠️ الرمز غير مدعوم أو فشل الاتصال بالسيرفر.")
...
# --- [ 6. تشغيل السيرفر والبوت بتوافق Render ] ---
def run_bot():
